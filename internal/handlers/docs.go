	...
	c.JSON(http.StatusOK, gin.H{
		...
		"endpoints": []gin.H{
			...
			},
			{
				...
				"path":        "/prices/movers",
				"auth":        true,
				"description": "SKUs whose price moved between two completed snapshots, ranked by delta. One price field per request, chosen with price_type. Includes language, printing, and condition names. Each result includes change_type, plus prev_snapshot_at and curr_snapshot_at as UTC RFC 3339 timestamps of the actual resolved snapshots, not individual listing-change times or the requested dates. Defaults to the two most recent completed snapshots. Prices and deltas are nullable: a SKU present in only one snapshot, or whose price appeared or disappeared, has no defined delta, and a missing price is never coerced to zero. Null deltas sort after every ranked mover. max_price is not implemented and is ignored.",
				"errors": gin.H{
