	...
	c.JSON(http.StatusOK, gin.H{
		...
		"endpoints": []gin.H{
			...
			},
			{
				...
				"path":        "/prices/movers",
				"auth":        true,
				"description": "Price changes between two completed snapshots. Per-SKU with all price fields. Defaults to the two most recent completed snapshots.",
				"params": gin.H{
					"from":        "start date YYYY-MM-DD — resolves to latest completed snapshot before midnight UTC of the following day",
				...
				"response_fields": gin.H{
					...
					"market_delta_cents":   "null when either side lacks a market price",
					"market_delta_percent": "null when prev is null or zero",
				},
			},
			{
				"method":      "GET",
